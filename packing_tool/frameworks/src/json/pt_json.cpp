...
bool PtJson::Add(const char *key, const std::list<std::string>& values) const
{
    if (key == nullptr || Contains(key)) {
        return false;
    }
...
Result PtJson::SetBool(const char *key, const bool& value)
{
    cJSON *item = cJSON_GetObjectItem(object_, key);
    if (item == nullptr) {
        return Result::NOT_EXIST;
...
Result PtJson::SetInt64(const char *key, const int64_t& value)
{
    cJSON *item = cJSON_GetObjectItem(object_, key);
    if (item == nullptr) {
        return Result::NOT_EXIST;
...
Result PtJson::SetUInt(const char *key, const uint32_t& value)
{
    cJSON *item = cJSON_GetObjectItem(object_, key);
    if (item == nullptr) {
        return Result::NOT_EXIST;
...
Result PtJson::SetUInt64(const char *key, const uint64_t& value)
{
    cJSON *item = cJSON_GetObjectItem(object_, key);
    if (item == nullptr) {
        return Result::NOT_EXIST;
...
Result PtJson::SetDouble(const char *key, const double& value)
{
    cJSON *item = cJSON_GetObjectItem(object_, key);
    if (item == nullptr) {
        return Result::NOT_EXIST;
...
Result PtJson::SetString(const char *key, const std::string& value)
{
    cJSON *item = cJSON_GetObjectItem(object_, key);
    if (item == nullptr) {
        return Result::NOT_EXIST;
