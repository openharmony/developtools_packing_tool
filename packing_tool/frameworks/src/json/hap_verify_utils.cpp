...
bool HapVerifyUtils::CheckHapIsValid(const std::list<HapVerifyInfo>& hapVerifyInfos)
{
    if (hapVerifyInfos.empty()) {
        ...
        LOGE("%s", PackingToolErrMsg::CHECK_HAP_VERIFY_INFO_LIST_EMPTY.toStringWithArgs("hapVerifyInfos is empty!").c_str());
        return false;
    }
    ...
    if (!CheckModuleNameIsValid(hapVerifyInfos)) {
        ...
        LOGE("%s", PackingToolErrMsg::CHECK_MODULE_NAME_INVALID.toStringWithArgs("CheckModuleNameIsValid failed!").c_str());
        return false;
    }
    if (!CheckPackageNameIsValid(hapVerifyInfos)) {
        // LOGE("CheckPackageNameIsValid failed! PackageName duplicated.");
        LOGE("%s", PackingToolErrMsg::CHECK_PACKAGE_NAME_INVALID.toStringWithArgs("CheckPackageNameIsValid failed! PackageName duplicated.").c_str());
        return false;
    }
    ...
    if (!CheckDependencyIsValid(hapVerifyInfos)) {
        ...
        LOGE("%s", PackingToolErrMsg::CHECK_DEPENDENCY_INVALID.toStringWithArgs("CheckDependencyIsValid failed!").c_str());
        return false;
    }
    ...
    if (!CheckTargetModuleNameIsExisted(hapVerifyInfos)) {
        ...
        LOGE("%s", PackingToolErrMsg::TARGET_MODULE_NAME_NOT_EXIST.toStringWithArgs("CheckTargetModuleNameIsExisted failed. Target module is not found.").c_str());
        return false;
    }
    if (!CheckCompileSdkIsValid(hapVerifyInfos)) {
        // LOGE("CheckCompileSdkIsValid failed. Compile sdk config is not same.");
        LOGE("%s", PackingToolErrMsg::COMPILE_SDK_TYPE_DIFFERENT.toStringWithArgs("CheckCompileSdkIsValid failed. Compile sdk config is not same.").c_str());
        return false;
    }
    if (!CheckProxyDataUriIsUnique(hapVerifyInfos)) {
        // LOGE("CheckProxyDataUriIsUnique failed. Uris in proxy data are not unique.");
        LOGE("%s", PackingToolErrMsg::PROXY_DATA_URI_NOT_UNIQUE.toStringWithArgs("CheckProxyDataUriIsUnique failed. Uris in proxy data are not unique.").c_str());
        return false;
    }
    if (!CheckContinueTypeIsValid(hapVerifyInfos)) {
        // LOGE("CheckContinueTypeIsValid failed.");
        LOGE("%s", PackingToolErrMsg::CONTINUE_TYPE_INVALID.toStringWithArgs("CheckContinueTypeIsValid failed.").c_str());
        return false;
    }
    return true;
}
...
bool HapVerifyUtils::CheckIsPluginApp(const std::list<HapVerifyInfo>& hapVerifyInfos)
{
    ...
    if (it != hapVerifyInfos.end()) {
        if (hapVerifyInfos.size() != 1) {
            ...
            LOGE("%s", PackingToolErrMsg::CHECK_HAP_INVALID.toStringWithArgs("plugin App must contain only one element").c_str());
            return false;
        }
        ...
        if (hapVerifyInfo.GetFileType() != HSP_SUFFIX) {
            ...
            LOGE("%s", PackingToolErrMsg::CHECK_HAP_INVALID.toStringWithArgs("plugin App must be of type hsp").c_str());
            return false;
        }
    }
    return true;
...
bool HapVerifyUtils::AppFieldsIsSame(const VerifyCollection& verifyCollection, const HapVerifyInfo& hapVerifyInfo)
{
    if (hapVerifyInfo.GetBundleName().empty()) {
        ...
        LOGE("%s", PackingToolErrMsg::CHECK_HAP_INVALID.toStringWithArgs("bundleName is empty").c_str());
        return false;
    }
    if (verifyCollection.bundleName != hapVerifyInfo.GetBundleName()) {
        // LOGE("input module bundleName is different.");
        LOGE("%s", PackingToolErrMsg::CHECK_HAP_INVALID.toStringWithArgs("input module bundleName is different.").c_str());
        return false;
    }
    if (verifyCollection.bundleType != hapVerifyInfo.GetBundleType()) {
        // LOGE("input module bundleType is different.");
        LOGE("%s", PackingToolErrMsg::CHECK_HAP_INVALID.toStringWithArgs("input module bundleType is different.").c_str());
        return false;
    }
    if (verifyCollection.versionCode != hapVerifyInfo.GetVersion().versionCode) {
        // LOGE("input module versionCode is different.");
        LOGE("%s", PackingToolErrMsg::CHECK_HAP_INVALID.toStringWithArgs("input module versionCode is different.").c_str());
        return false;
    }
    if (verifyCollection.buildVersion != hapVerifyInfo.GetVersion().buildVersion) {
        // LOGE("input module buildVersion is different.");
        LOGE("%s", PackingToolErrMsg::CHECK_HAP_INVALID.toStringWithArgs("input module buildVersion is different.").c_str());
    ...
    if (verifyCollection.releaseType != hapVerifyInfo.GetApiVersion().releaseType) {
        ...
        LOGE("%s", PackingToolErrMsg::CHECK_HAP_INVALID.toStringWithArgs("input module releaseType is different.").c_str());
        // LOGE("Solutions: > Check if the releaseType" \
        //     " is the same in different modules.");
        LOGE("%s", PackingToolErrMsg::CHECK_HAP_INVALID.toStringWithArgs("Solutions: > Check if the releaseType is the same in different modules.").c_str());
        return false;
    }
    if (verifyCollection.targetBundleName != hapVerifyInfo.GetTargetBundleName()) {
        // LOGE("targetBundleName is different.");
        LOGE("%s", PackingToolErrMsg::CHECK_HAP_INVALID.toStringWithArgs("targetBundleName is different.").c_str());
        return false;
    }
    if (verifyCollection.targetPriority != hapVerifyInfo.GetTargetPriority()) {
        // LOGE("targetPriority is different.");
        LOGE("%s", PackingToolErrMsg::CHECK_HAP_INVALID.toStringWithArgs("targetPriority is different.").c_str());
        return false;
    }
    if (IsEntryOrFeature(verifyCollection.moduleType) && IsEntryOrFeature(hapVerifyInfo.GetModuleType())) {
        if (verifyCollection.multiAppMode != (hapVerifyInfo.GetMultiAppMode())) {
            ...
            LOGE("%s", PackingToolErrMsg::CHECK_HAP_INVALID.toStringWithArgs("multiAppMode is different.").c_str());
            return false;
        }
    }
    return true;
    ...
    const std::string& tip)
{
    ...
    LOGE("%s", PackingToolErrMsg::CHECK_HAP_INVALID.toStringWithArgs(std::string("Solution: Make sure the ") + tip + " is valid and unique.").c_str());
    // LOGE("Reference: %s.", REFERENCE_LINK.c_str());
    LOGE("%s", PackingToolErrMsg::CHECK_HAP_INVALID.toStringWithArgs(std::string("Reference: ") + REFERENCE_LINK + ".").c_str());
}
...
bool HapVerifyUtils::CheckModuleNameIsValid(const std::list<HapVerifyInfo>& hapVerifyInfos)
{
    for (auto iter1 = hapVerifyInfos.begin(); iter1 != hapVerifyInfos.end(); iter1++) {
        ...
        if (hapVerifyInfo1.GetModuleName().empty()) {
            ...
            LOGE("%s", PackingToolErrMsg::CHECK_HAP_INVALID.toStringWithArgs("module name can not be empty.").c_str());
            return false;
        }
...
bool HapVerifyUtils::CheckPolicyValueDisjoint(const PolicyValue& policyValue1, const PolicyValue& policyValue2)
{
    if (policyValue1.policy == EXCLUDE && policyValue2.policy == INCLUDE) {
        ...
        LOGE("%s", PackingToolErrMsg::CHECK_HAP_INVALID.toStringWithArgs("input policys are both exclude.").c_str());
        return false;
    } else {
        // LOGE("input policy is invalid.");
        LOGE("%s", PackingToolErrMsg::CHECK_HAP_INVALID.toStringWithArgs("input policy is invalid.").c_str());
    }
    return false;
}
...
bool HapVerifyUtils::CheckApiVersionCovered(const ApiVersion& apiVersion, const std::list<HapVerifyInfo>& entryHaps)
{
    ...
    for (auto& hapVerifyInfo : entryHaps) {
        ...
        if (hapVerifyInfo.GetDistroFilter().apiVersion.policy.empty()) {
            ...
            LOGE("%s", PackingToolErrMsg::CHECK_HAP_INVALID.toStringWithArgs("input none policy.").c_str());
            return false;
        }
...
bool HapVerifyUtils::CheckDependencyIsValid(const std::list<HapVerifyInfo>& hapVerifyInfos)
{
    if (hapVerifyInfos.empty()) {
        // LOGE("input none hap.");
        LOGE("%s", PackingToolErrMsg::CHECK_HAP_INVALID.toStringWithArgs("input none hap.").c_str());
        return false;
    }
    ...
    for (auto& hapVerifyInfo : hapVerifyInfos) {
        if (isInstallationFree != hapVerifyInfo.IsInstallationFree()) {
            ...
            LOGE("%s", PackingToolErrMsg::CHECK_HAP_INVALID.toStringWithArgs("installationFree is different in input hap.").c_str());
            return false;
        }
    }
    for (auto& hapVerifyInfo : hapVerifyInfos) {
...
bool HapVerifyUtils::CheckSharedAppIsValid(const std::list<HapVerifyInfo>& hapVerifyInfos)
{
    ...
    for (auto& hapVerifyInfo : hapVerifyInfos) {
        if (hapVerifyInfo.GetModuleName() != moduleName) {
            LOGE("%s", PackingToolErrMsg::CHECK_MODULE_NAME_INVALID.toStringWithArgs("moduleName is not same").c_str());
            return false;
        }
        if (!hapVerifyInfo.GetDependencyItemList().empty()) {
            LOGE("%s", PackingToolErrMsg::CHECK_DEPENDENCY_INVALID.toStringWithArgs("dependencyItems is empty").c_str());
            return false;
        }
    ...
    for (auto iter1 = hapVerifyInfos.begin(); iter1 != hapVerifyInfos.end(); iter1++) {
        for (auto iter2 = std::next(iter1); iter2 != hapVerifyInfos.end(); iter2++) {
            if (!CheckDuplicatedIsValid(*iter1, *iter2)) {
                LOGE("%s", PackingToolErrMsg::CHECK_HAP_INVALID.toStringWithArgs("CheckDuplicatedIsValid failed!").c_str());
                return false;
            }
        }
    }
